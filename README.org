...
{:year        (year)
 :name        project
 :sanitized   (sanitize-ns project)
 :main-ns     (multi-segment (sanitize-ns project))
 :path        (name-to-path (multi-segment (sanitize-ns project)))
 :group-id    (group-name project)
 :artifact-id (project-name project)}
     #+END_SRC
